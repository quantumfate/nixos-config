...
if [[ "$1" == "--upgrade" || "$1" == "-u" ]]; then
    ...
    pkill zen-twilight
    sudo nixos-rebuild switch --flake $ADDITIONAL_FLAGS "$HOME/nixos-config#$HOST"
    ...
    git push
    pkill zen-twilight
    sudo nixos-rebuild switch --flake --upgrade $ADDITIONAL_FLAGS "$HOME/nixos-config#$HOST"
else
