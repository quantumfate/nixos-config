...
let
  ...
  toRgba =
    color: alpha: "rgba(${toString color.r},${toString color.g},${toString color.b},${toString alpha})";
  ...
  calc_rgba_hex = color: alpha: "${color}" + lib.toHexString (builtins.floor (alpha * 255));

  rgba_bare_hexify =
    colors:
    lib.mapAttrs (
      _name: color: alpha:
      lib.strings.toUpper (calc_rgba_hex color alpha)
    ) colors;
  ...
  rgba_hexify =
    ...
    lib.mapAttrs (
      _name: color: alpha:
      toHex (calc_rgba_hex color alpha)
    ) colors;
