...
let
  ...
  formatRgbForHyprland = rgbString:
    # 1. Strip the surrounding "rgb(" and ")"
    let
      content = builtins.substring 4 (builtins.stringLength rgbString - 5) rgbString;
    in
    # 2. Replace all remaining spaces with an empty string
    "rgb(${lib.strings.replaceStrings [ " " ] [ "" ] content})";
